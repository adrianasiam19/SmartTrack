...
def test_balanced_psychometric_selection_is_unique_and_varied():
    ...
    selected = _balanced_psychometric_questions(cards, count=6, seen_card_ids=set())
    ...
    selected_again = _balanced_psychometric_questions(
        cards, count=6, seen_card_ids=blocked
    )
...
def test_adaptive_prompt_contains_rich_context():
    prompt = build_adaptive_cognitive_prompt(
        ...
        covered_categories=["Learning Preferences", "Decision Making"],
        skill_plan=["creativity", "critical thinking", "problem solving"],
        format_plan=["short-response", "best-solution", "scenario"],
    )
