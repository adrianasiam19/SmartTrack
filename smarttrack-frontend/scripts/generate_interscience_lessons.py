#!/usr/bin/env python3
"""
generate_interscience_lessons.py
─────────────────────────────────
Reads extracted Integrated Science SHS 1 texts from Downloads/Inter Science 1/
...
SOURCE_DIR = r"C:\Users\Admin\Downloads\Inter Science 1"
OUTPUT_FILE = Path(__file__).resolve().parent.parent / "app" / "lib" / "generatedInterScienceLessons.ts"

SECTIONS = [
    {"id": "s1", "title": "Exploring Materials — Characteristics of Science",
     "file": "LM-General-Science-section-1-Lversion (1).txt",
     "icon": "🔬", "color": "#059669",
     "subtopics": ["What is Science?", "Characteristics of Science", "Scientific Inquiry and Design", "Applications of Science in Daily Life"]},
    {"id": "s2", "title": "Science and Materials in Nature",
     "file": "LM-General-Science-section-2-LVersion (2).txt",
     "icon": "⚗️", "color": "#0D9488",
     "subtopics": ["The Periodic Table of Elements", "Metals and Their Properties", "Non-metals and Semi-metals", "Binary Compounds"]},
    {"id": "s3", "title": "Diffusion and Osmosis",
     "file": "LM-General-Science-section-3-LVersion (1).txt",
     "icon": "💧", "color": "#0284C7",
     "subtopics": ["Concepts of Diffusion", "Factors Affecting Diffusion", "Understanding Osmosis", "Applications of Diffusion and Osmosis"]},
    {"id": "s4", "title": "Reproduction in Plants and Humans",
     "file": "LM-General-Science-section-4-LVersion (1).txt",
     "icon": "🌱", "color": "#65A30D",
     "subtopics": ["Reproduction in Plants", "Pollination and Fertilisation", "Human Reproductive System", "The Menstrual Cycle"]},
    {"id": "s5", "title": "Solar Panels",
     "file": "LM-General-Science-section-5-LVersion (1).txt",
     "icon": "☀️", "color": "#D97706",
     "subtopics": ["Introduction to Solar Energy", "How Solar Panels Work", "Solar Energy in Ghana", "Building Solar Panels"]},
    {"id": "s6", "title": "Force",
     "file": "LM-General-Science-section-6-LVersion.txt",
     "icon": "⚡", "color": "#DC2626",
     "subtopics": ["Types of Forces", "Frictional Forces", "Gravitational Force", "Velocity and Acceleration"]},
    {"id": "s7", "title": "Basic Electronics",
     "file": "LM-General-Science-Section-7-LVersion (1).txt",
     "icon": "🔌", "color": "#7C3AED",
     "subtopics": ["Electronic Components", "Resistors and Capacitors", "Diodes and Transistors", "Building Circuits"]},
    {"id": "s8", "title": "Promoting Health and Safety",
     "file": "LM-General-Science-Section-8-LVersion (1).txt",
     "icon": "🏥", "color": "#E11D48",
     "subtopics": ["Hazards in the Environment", "Chemical and Biological Hazards", "Lifestyle Diseases", "Drugs and Their Effects"]},
    {"id": "s9", "title": "Production in Local Industry",
     "file": "LM-General-Science-Section-9-LVersion (1).txt",
     "icon": "🧼", "color": "#A21CAF",
     "subtopics": ["Local Soap Production", "The Saponification Process", "Types of Soap", "Local Food Production"]},
]
...
def clean_extracted_text(text: str) -> str:
    ...
    for line in lines:
        ...
        if s.startswith('SECTION '):
            continue
        if 'General Science' in s and 'Year' in s:
            continue
        if not s or len(s) < 3:
            continue
...
def build_lesson_content(paragraphs: list[str], section: dict, lesson_idx: int, subtopic: str) -> dict:
    ...
    lesson_id = f"int-sci-{section['id']}t{lesson_idx}"
    steps = []
    ...
    for para in paragraphs[1:4]:  # First 3 content paragraphs
        if len(para) > 30:
            ...
            is_activity = any(kw in para_clean.upper() for kw in ["ACTIVITY", "EXPERIMENT", "EXERCISE"])
            if is_activity:
                steps.append({
...
def build_prerequisites(section_idx: int, lesson_idx: int) -> list[str]:
    ...
    if section_idx == 1 and lesson_idx == 1:
        return prereqs
    if lesson_idx > 1:
        prereqs.append(f"int-sci-s{section_idx}t{lesson_idx - 1}")
    else:
        prev_section = section_idx - 1
        prev_count = len(SECTIONS[prev_section - 1]["subtopics"])
        prereqs.append(f"int-sci-s{prev_section}t{prev_count}")
    return prereqs
...
def main():
    print("=" * 60)
    print("  Integrated Science SHS 1 Lesson Generator")
    print("=" * 60)
